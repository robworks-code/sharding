...
function loadManifest(root) {
  ...
  const raw = (0, import_yaml.parse)(readFileSync2(path, "utf8"));
  const shards = {};
  for (const [name, entry] of Object.entries(raw.shards ?? {})) {
    shards[name] = {
      dir: entry.dir,
...
function shapeLabel(s) {
  switch (s.kind) {
    ...
    case "object":
      return "object";
    case "array":
      return `array<${shapeLabel(s.items)}>`;
    case "enum":
      return `enum(${s.values.join("|")})`;
    case "ref":
      return `ref:${s.name}`;
  }
}
function diffShape(expected, actual, slice, loc) {
  ...
  if (expected.kind === "primitive" && actual.kind === "primitive") {
    return expected.name === actual.name ? [] : [{ slice, kind: "type-mismatch", location: loc, expected: expected.name, actual: actual.name }];
  }
  if (expected.kind === "ref" && actual.kind === "ref") {
    return expected.name === actual.name ? [] : [{ slice, kind: "type-mismatch", location: loc, expected: `ref:${expected.name}`, actual: `ref:${actual.name}` }];
  }
  return [];
}
function diffSymbol(expected, actual, slice) {
  if (expected.kind !== actual.kind) {
...
// src/conventions/lint.ts
function lintConventions(surface, rules) {
  const findings = [];
...
function checkPhase(root, runAcceptance = defaultRunner) {
  const manifest = loadManifest(root);
...
function isWriteAllowed(shardDir, contractDir, target) {
  return isInside(shardDir, target) && !isInside(contractDir, target);
}
...
function sliceLines(adapterName, shardDir, slices, role) {
  if (slices.length === 0) return "  (none)";
  return slices.map((slice) => `  - ${slice} -> ${surfacePathFor(adapterName, shardDir, slice, role)}`).join("\n");
}
function buildShardPrompt(root, shard, options = {}) {
...
function run(argv, cwd) {
  ...
  switch (cmd) {
    ...
    case "sandbox-check": {
      const f = flags(rest);
      const allowed = f.mode === "write" ? isWriteAllowed(f["shard-dir"], f["contract-dir"], f.target) : isReadAllowed(f["shard-dir"], f["contract-dir"], f.target);
