...
3. If the check reports `versionStale: false`, report "already acknowledged against <version>" and stop.
4. If it reports drift (exit non-zero), stop: the drift must be resolved first. `ack` will refuse anyway.
