...
3. **Halt** - if `haltedAfterWave` is set, say so and name the waves that were never dispatched. Their shards' verdicts describe whatever was already on disk, not the result of a session, and reporting them without that caveat reads as if they ran and passed.
