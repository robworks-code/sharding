...
3. **Gate** - the `/shard-phase-check` result: shards clean, versions acknowledged, acceptance suite. If `gate` is `null` the gate was skipped, so do not describe the phase as passing.
