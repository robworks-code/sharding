...
- `cyclic` - shards with no order relative to each other because they consume each other's slices. Legal (they couple through the contract, not through each other), so report it as "no meaningful order, scheduled together" rather than as a failure.
