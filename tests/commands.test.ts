...
describe("plugin command surface", () => {
  it("ships all seven commands and the skill", () => {
    for (const n of ["shard-init", "shard-contract", "shard-new", "shard-check", "shard-ack", "shard-phase-check", "shard-status"]) {
      expect(existsSync(join(root, "commands", `${n}.md`))).toBe(true);
    }
    expect(existsSync(join(root, "skills", "sharding", "SKILL.md"))).toBe(true);
  });
  it("check/ack/status/phase-check commands invoke the matching CLI subcommand", () => {
